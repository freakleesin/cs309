...
public class UsersController extends BaseController {
	...
	@ResponseBody
	public ComReturnType register(@RequestParam("username") String username, @RequestParam("email") String email,
			@RequestParam("password") String password, @RequestParam("usersType") String usersType)
	...
	@ResponseBody
	public ComReturnType login(@RequestParam("username") String username, @RequestParam("password") String password)
			throws BusException, NoSuchAlgorithmException, UnsupportedEncodingException {
	...
	@ResponseBody
	public ComReturnType updateUsername(@RequestParam("username") String username,
			@RequestParam("newusername") String newUsername)
					throws BusException, NoSuchAlgorithmException, UnsupportedEncodingException {

		if (StringUtils.isEmpty(username) || StringUtils.isEmpty(newUsername)) {
			throw new BusException(EnumBusError.PARAMETER_VALIDATIOIN_ERROR);
		}
		// update username.
		Integer result = usersService.updateUsername(username, newUsername);
	...
	@ResponseBody
	public ComReturnType updatePassword(@RequestParam("username") String username,
			@RequestParam("password") String oldPassword, @RequestParam("newpassword") String newPassword)
					throws BusException, NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		// update password.
		Integer result = usersService.updatePassword(username, this.EncodeByMd5(oldPassword),
				this.EncodeByMd5(newPassword));
		return ComReturnType.create(result);
	}

	@RequestMapping(value = "/like", method = { RequestMethod.POST })
	@ResponseBody
	public ComReturnType like(@RequestParam("username") String username) {
		
		usersService.like(username);
		return ComReturnType.create(null);
	}
	...
	@RequestMapping("/get")
	@ResponseBody
	public ComReturnType getUsers(@RequestParam("id") String id) throws BusException {
	...
	private UsersVO convertFromModel(UsersModel usersModel) {
		...
		usersVO.setId(usersModel.getId());
		usersVO.setUsername(usersModel.getUsername());
		usersVO.setEmail(usersModel.getEmail());
		usersVO.setUsersType(usersModel.getUsersType());
		usersVO.setLikeNum(usersModel.getLikeNum());
		return usersVO;
	}
